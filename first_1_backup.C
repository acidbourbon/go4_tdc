...
void first()
{
  //base::ProcMgr::instance()->SetRawAnalysis(true);
   ...
   hadaq::TdcMessage::SetFineLimits(10, 610);
   ...
   int cnt = (calmode && *calmode) ? atoi(calmode) : 300000000000;
...
extern "C" void after_create(hadaq::HldProcessor* hld)
{
   ...
   for (unsigned k=0;k<hld->NumberOfTDC();k++) {
      ...
      tdc->SetLinearCalibration(1, 16, 506);
      tdc->SetLinearCalibration(2, 17, 506);
      tdc->SetLinearCalibration(3, 16, 500);
      tdc->SetLinearCalibration(4, 16, 504);
      tdc->SetLinearCalibration(5, 17, 508);
      tdc->SetLinearCalibration(6, 16, 509);
      tdc->SetLinearCalibration(7, 16, 498);
      tdc->SetLinearCalibration(8, 17, 498);
	...
	tdc->SetRefChannel(0,0, 0x0203, 20000, -10, 10); // LED DIFF
	tdc->SetRefChannel(1,16, 0x0203, 20000, -17000, 17000); // LED DIFF
	tdc->SetRefChannel(2,1, 0x0203, 20000, -50, 50); // LED DIFF
	tdc->SetRefChannel(3,2, 0x0203, 20000, -50, 50); // LED DIFF
  tdc->SetRefChannel(4,1, 0x0203, 20000, -50, 50); // LED DIFF
  tdc->SetRefChannel(5,4, 0x0203, 20000, -50, 50); // LED DIFF
